...
if(a.equals("insert")){
    // 执行插入电子邮件模块,表名：dianziyoujian
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
                                            
    String charuid = new CommDAO().insert(request,"dianziyoujian",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
new CommDAO().commOper("update dingdanxinxi set zhuangtai='已发货' where id='"+request.getParameter("dingdanxinxiid")+"'");
    ...
    // 执行更新电子邮件模块,表名：dianziyoujian
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
                                                                                                                                

    String charuid = new CommDAO().update(request,"dianziyoujian",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    HashMap map = Query.make("dianziyoujian").find(Request.get("id"));
    
    new CommDAO().delete(request,"dianziyoujian");  // 执行删除命令
