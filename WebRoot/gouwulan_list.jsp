...
// 以下是检测搜索框中是否填写了或者选择了什么，则写入sql 语句中
 ...
 if(request.getParameter("xiaoji_end") != null && !"".equals( request.getParameter("xiaoji_end") )){ where+= " AND xiaoji <='"+request.getParameter("xiaoji_end")+"' "; }        List<HashMap> list = Query.make("gouwulan").where(where).order(orderby+" "+sort).page(15);
...
<td> <% 
 HashMap mapfenlei7 = new CommDAO().find("SELECT fenleimingcheng FROM fenlei where id='"+map.get("fenlei")+"'"); 
 %><%= mapfenlei7.get("fenleimingcheng") %> </td>
<td> <%= map.get("guige") %> </td>
<td> <%= map.get("jiage") %> </td>
...
<td> <%= map.get("goumairen") %> </td>
                                                    <td>
                                                                    <%= map.get("issh") %>
                                    <a href="sh.jsp?id=<%= map.get("id") %>&yuan=<%= map.get("issh") %>&tablename=gouwulan">
