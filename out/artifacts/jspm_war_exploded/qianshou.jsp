...
if(a.equals("insert")){
    // 执行插入签收模块,表名：qianshou
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
                                    
    String charuid = new CommDAO().insert(request,"qianshou",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
new CommDAO().commOper("update dingdanxinxi set zhuangtai='已签收' where id='"+request.getParameter("dingdanxinxiid")+"'");
    ...
    // 执行更新签收模块,表名：qianshou
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
                                                                                                        

    String charuid = new CommDAO().update(request,"qianshou",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    HashMap map = Query.make("qianshou").find(Request.get("id"));
    
    new CommDAO().delete(request,"qianshou");  // 执行删除命令
